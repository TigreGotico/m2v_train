...
# catalan model, use all loaded datasets for training
...
# Initialize a classifier from one of the catalan models
for base_model in base_models:
    classifier = StaticModelForClassification.from_pretrained(model_name=base_model)

    # Train the catalan classifier on the combined data
    logger.info(f"Training catalan model {base_model}...")
    classifier.fit(X_train, y_train, max_epochs=25)

    # Predict using the trained catalan classifier
    logger.info(f"Predicting using the catalan model {base_model}...")
    y_pred = classifier.predict(X_test)

    # Evaluate the catalan model using various metrics
    logger.info(f"Evaluating catalan model {base_model}...")
    ...
    logger.info(f"catalan Classification report:")
    ...
    # Save the trained catalan model
    catalan_model_path = f"model_ca_{base_model.split('/')[-1]}"
    pipeline = classifier.to_pipeline()

    if os.path.isfile(catalan_model_path):
        os.remove(catalan_model_path)
    pipeline.save_pretrained(catalan_model_path)
    logger.info(f"catalan Model saved as {catalan_model_path}")

    # Save the evaluation metrics for the catalan model
    summary = {
        "language": "catalan",
        "model": base_model,
        "accuracy": accuracy,
    ...
    with open(f"metrics_ca_{base_model.split('/')[-1]}.md", "w") as f:
    ...
    plt.tight_layout()
    plt.savefig(f"ca_{base_model.split('/')[-1]}_f1_per_class.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"ca_{base_model.split('/')[-1]}_top20_misclassified_intents.png")
    plt.close()
...
with open("model_comparison_ca.md", "w") as f:
