...
# portuguese model, use all loaded datasets for training
...
# Initialize a classifier from one of the portuguese models
for base_model in base_models:
    classifier = StaticModelForClassification.from_pretrained(model_name=base_model)

    # Train the portuguese classifier on the combined data
    logger.info(f"Training portuguese model {base_model}...")
    classifier.fit(X_train, y_train, max_epochs=25)

    # Predict using the trained portuguese classifier
    logger.info(f"Predicting using the portuguese model {base_model}...")
    y_pred = classifier.predict(X_test)

    # Evaluate the portuguese model using various metrics
    logger.info(f"Evaluating portuguese model {base_model}...")
    ...
    logger.info(f"portuguese Classification report:")
    ...
    # Save the trained portuguese model
    portuguese_model_path = f"model_pt_{base_model.split('/')[-1]}"
    pipeline = classifier.to_pipeline()

    if os.path.isfile(portuguese_model_path):
        os.remove(portuguese_model_path)
    pipeline.save_pretrained(portuguese_model_path)
    logger.info(f"portuguese Model saved as {portuguese_model_path}")

    # Save the evaluation metrics for the portuguese model
    summary = {
        "language": "portuguese",
        "model": base_model,
        "accuracy": accuracy,
    ...
    with open(f"metrics_pt_{base_model.split('/')[-1]}.md", "w") as f:
    ...
    f1_per_class = {label: report_dict.get(label, {}).get("f1-score", 0.0)
                    for label in valid_labels if label in report}
    ...
    plt.tight_layout()
    plt.savefig(f"pt_{base_model.split('/')[-1]}_f1_per_class.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"pt_{base_model.split('/')[-1]}_top20_misclassified_intents.png")
    plt.close()
...
with open("model_comparison_pt.md", "w") as f:
