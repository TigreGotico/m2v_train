...
for base_model in base_models:
    ...
    english_model_path = f"model_en_{base_model.split('/')[-1]}"
    pipeline = classifier.to_pipeline()

    if os.path.isfile(english_model_path):
        os.remove(english_model_path)
    pipeline.save_pretrained(english_model_path)
    logger.info(f"english Model saved as {english_model_path}")
    ...
    plt.tight_layout()
    plt.savefig(f"en_{base_model.split('/')[-1]}_f1_per_class.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"en_{base_model.split('/')[-1]}_top20_misclassified_intents.png")
    plt.close()
